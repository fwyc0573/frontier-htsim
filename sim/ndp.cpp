...
void NdpSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    // Conservative guard to prevent spurious timeout events/logs:
    // _sent_times tracks outstanding (sent-but-not-yet-acked/nacked) packets for THIS flow.
    // Timeout-based retransmission is driven by overdue entries in _sent_times.
    //
    // In some scenarios, _rtx_timeout can remain stale even when _sent_times becomes empty:
    // - Flow completion: processAck() may early-return when cum_ackno >= _flow_size, skipping
    //   the usual update_rtx_time() call that would clear _rtx_timeout when _sent_times is empty.
    // - Packets moved out of _sent_times: e.g., outstanding packets can be removed from _sent_times
    //   and queued in _rtx_queue (to be resent later via the normal sending path). In this case,
    //   there may be work in _rtx_queue, but there is no timeout-based work tied to _sent_times.
    //
    // Without this guard, a stale _rtx_timeout could cause spurious "At ... RTO ..." debug prints
    // and unnecessary timeout events, even though there are no outstanding packets to retransmit.
    //
    // This is consistent with update_rtx_time(): when _sent_times is empty, _rtx_timeout should be timeInf.
    //
    // IMPORTANT: This does NOT change normal retransmission behavior because real retransmits are
    // triggered only when _sent_times contains overdue entries. If _sent_times is empty, there is
    // nothing to retransmit via the timeout mechanism.
    if (_sent_times.empty()) {
        _rtx_timeout = timeInf;
        return;
    }
