...
# -*- coding: utf-8 -*-
"""
rail_ocs3_schedule_dsl.py

Goal: make OCS schedules human-readable via a DSL, then compile to the existing .schedule format:
  slot 0
  plane 0 a-b c-d ...

Note:
- This is an "offline" tool, not automatically called by the runner.
- Designed to be minimal: supports the two port DSLs used in this document:
    tor(<tor>).up(<up>)
    trunk(<i>)
  and core-ocs:
    pod(<p>).trunk(<i>)
- You can extend the port types as needed.

Usage examples:
  python3 rail_ocs3_schedule_dsl.py --in agg_ocs_tor_trunk...dsl --out agg_ocs_tor_trunk...schedule
  python3 rail_ocs3_schedule_dsl.py --in core_ocs_ring...dsl --out core_ocs_ring...schedule

This tool (and the example DSLs in sim/datacenter/topologies/) is intended for the
upcoming **Rail-OCS3 (3-layer, two-OCS, B1)** design. It is NOT the legacy 2-layer rail_ocs.
"""
